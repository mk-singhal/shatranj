...
export default function UserBlog() {
  ...
  const handleSearchNSort = (sort = "", search = "") => {
    ...
    else navigate(`/blog/user/${username}`);
  };
  ...
  const getblogs = async () => {
    try {
      const response = await axios.get(
        `/blog/user/${username}?offset=0&limit=${limit}&search=${searchString}&sort=${sortBy}`
      );
      response.data?.blogs ? setBlogs(response.data.blogs) : setBlogs([]);
      response.data?.user ? setUser(response.data.user) : setUser(null);
  ...
  const fetchBlogs = React.useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `/blog/user/${username}?offset=${index}&limit=${limit}&search=${searchString}&sort=${sortBy}`
      );
          ...
          >
            ...
            {tab === "profile" && (
              <Grid container>
                ...
                <Grid item md={8} sx={{ px: { xs: 2, md: 3 }, pt: 1 }}>
                  <Typography variant="h5">About</Typography>
                  <Typography variant="subtitle1">
                    “I feel sorry for players who are always lying awake at
                    night, brooding over their games.”
                    <br /> — Magnus Carlsen
                  </Typography>
